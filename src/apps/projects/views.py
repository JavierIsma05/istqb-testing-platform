...
def build_project_card(project):
    ...
    student = project.created_by if project.created_by and project.created_by.role == User.Roles.STUDENT else next((member for member in members if member.role == User.Roles.STUDENT), None)
    tutor = project.tutor or next((member for member in members if member.role == User.Roles.TEACHER), None) or project.created_by or next(iter(members), None)
    return {'project': project, 'coverage': coverage, 'total_cases': total_cases, 'passed_cases': passed_cases, 'defect_count': project.defect_count or 0, 'student': student, 'tutor': tutor, 'badge': STATUS_BADGES.get(project.status, 'muted')}
...
def project_list_view(request):
    ...
    if query:
        projects = projects.filter(Q(name__icontains=query) | Q(code__icontains=query) | Q(description__icontains=query))
    if status:
        projects = projects.filter(status=status)
    if member_id:
        selected_member_filter = get_object_or_404(User, pk=member_id)
        if selected_member_filter.role == User.Roles.STUDENT:
            projects = projects.filter(Q(members=selected_member_filter) | Q(created_by=selected_member_filter)).distinct()
        else:
    ...
    if request.user.role == User.Roles.TEACHER:
        filter_users = User.objects.filter(Q(role=User.Roles.STUDENT), Q(projects__in=projects) | Q(project_created__in=projects)).distinct().order_by('first_name', 'last_name', 'email')
    ...
    return render(request, 'projects/index.html', {'project_cards': project_cards, 'status_choices': Project.Status.choices, 'selected_status': status, 'selected_member': member_id, 'query': query, 'filter_users': filter_users, 'member_filter_label': member_filter_label, 'view_mode': view_mode, 'can_manage': can_manage_artifacts(request.user)})
...
def project_create_view(request):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        if project.status == Project.Status.ACTIVE and visible_projects_for(request.user, request=request).filter(status=Project.Status.ACTIVE).exists():
            form.add_error('start_date', 'Ya tienes un proyecto activo. Cierra o pausa el proyecto activo antes de crear otro activo.')
        else:
            ...
            log_action(request.user, 'CREATE', 'Project', project.pk, {'code': project.code, 'name': project.name, 'status': project.status, 'tutor_id': tutor.pk if tutor else None})
            return redirect('projects:detail', pk=project.pk)
    return render(request, 'projects/form.html', {'form': form, 'title': 'Nuevo Proyecto', 'automatic_status_label': Project.Status.PLANNED.label, 'subtitle': 'Registra un proyecto de titulación. El estado inicial se asigna automaticamente.'})
...
def project_edit_view(request, pk):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        log_action(request.user, 'UPDATE', 'Project', updated.pk, {'code': updated.code, 'name': updated.name, 'status': updated.status, 'tutor_id': tutor.pk if tutor else None})
    ...
    return render(request, 'projects/form.html', {'form': form, 'project': project, 'title': 'Editar Proyecto', 'automatic_status_label': project.get_status_display(), 'subtitle': 'Actualiza la información del proyecto. El código no puede modificarse.'})
...
def project_detail_view(request, pk):
    ...
    coverage = round((passed_cases / total_cases) * 100) if total_cases else 0
    return render(request, 'projects/detail.html', {'project': project, 'coverage': coverage, 'test_plans_count': test_plans.count(), 'requirements_count': requirements.count(), 'test_cases_count': total_cases, 'executions_count': executions.count(), 'defects_count': defects.count(), 'recent_requirements': requirements[:5], 'recent_test_cases': test_cases[:5]})
