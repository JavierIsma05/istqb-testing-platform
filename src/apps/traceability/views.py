...
def reopen_test_case_view(request, pk):
    ...
    test_case = get_object_or_404(TestCase.objects.select_related('test_plan__project'), pk=pk, test_plan__project__in=visible_projects_for(request.user, request=request))
    ...
    log_action(request.user, 'UPDATE', 'TestCase', test_case.pk, {'project_id': test_case.test_plan.project_id, 'test_case_id': test_case.pk, 'status': test_case.status, 'source': 'traceability_reopen_for_reexecution'})
    messages.success(request, f'Caso {test_case.code} reabierto para una nueva ejecución. El historial anterior se conserva.')
    return redirect(f'/executions/?case={test_case.pk}')
...
def traceability_matrix_view(request):
    ...
    for requirement in requirements:
        ...
        for test_case in test_cases:
            if test_case.latest_completed_pk: latest_execution_ids.add(test_case.latest_completed_pk)
    ...
    rows = []
    plans = set(); cases = set(); defects = set(); result_count = 0
    for requirement, test_cases in requirement_cases:
        ...
        for test_case in test_cases:
            ...
            plan = test_case.test_plan; plans.add(plan); cases.add(test_case)
            if execution: result_count += 1
            row_defects = list(test_case.defects.all())
            if execution: row_defects += list(execution.defects.all())
            defects.update(row_defects)
            rows.append({'requirement': requirement, 'plan': plan, 'case': test_case, 'execution': execution, 'defects': row_defects, 'rationale': getattr(link_by_case.get(test_case.pk), 'rationale', '') or ''})
    ...
    return render(request, 'traceability/index.html', {'rows': rows, 'project_metrics': project_metrics, 'total_requirements': len(requirements), 'total_plans': len(plans), 'total_test_cases': len(cases), 'total_executions': total_execution_history, 'latest_execution_snapshots': len(latest_executions), 'total_results': result_count, 'total_defects': len(defects), 'traced_requirements': len(traced_requirement_ids), 'requirements_with_completed_execution': len(requirements_with_completed_execution), 'requirement_coverage_percentage': requirement_coverage_percentage, 'execution_coverage_percentage': execution_coverage_percentage, 'end_to_end_traced_requirements': len(requirements_with_completed_execution), 'end_to_end_traceability_percentage': execution_coverage_percentage, 'executed_test_cases': len(executed_test_case_ids), 'test_case_execution_coverage_percentage': round((len(executed_test_case_ids) / len(cases)) * 100, 1) if cases else 0, 'can_manage': can_manage_artifacts(request.user)})