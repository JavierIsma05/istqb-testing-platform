...
def create_defect_from_failed_execution(execution):
    ...
    log_action(
        execution.executed_by, 'CREATE', 'Defect', defect.pk,
        {'project_id': project.pk, 'code': defect.code, 'title': defect.title,
         'status': defect.status, 'source': 'failed_execution', 'execution_id': execution.pk},
    )
    return defect
...
def sync_defect_from_confirmation(execution):
    ...
    log_action(
        execution.executed_by, 'UPDATE', 'Defect', defect.pk,
        {'project_id': defect.project_id, 'code': defect.code, 'status': defect.status,
         'source': 'confirmation_execution', 'execution_id': execution.pk},
    )
...
def build_execution_calendar(projects):
    ...
    recorded_executions = TestExecution.objects.select_related(
        'test_case', 'test_case__test_plan', 'test_case__test_plan__project', 'related_defect',
    ).filter(test_case__test_plan__project__in=projects, executed_at__isnull=False).order_by('-executed_at')[:50]
    for execution in recorded_executions:
        rows.append({'date': timezone.localtime(execution.executed_at).date(),
                     'time': timezone.localtime(execution.executed_at).time(),
                     'type': execution.get_execution_type_display(),
                     'project': execution.test_case.test_plan.project, 'test_case': execution.test_case,
                     'defect': execution.related_defect or execution.defects.first(),
                     'reason': f'Ejecución registrada: {execution.get_result_display()}', 'result': execution.result,
                     'result_label': execution.get_result_display(), 'is_recorded': True, 'execution': execution})
    pending_cases = TestCase.objects.select_related('test_plan', 'test_plan__project', 'requirement').filter(
        test_plan__project__in=projects, status__in=[TestCase.Status.PENDING, TestCase.Status.BLOCKED]
    ).order_by('test_plan__project__end_date', 'priority', 'code')[:20]
    for index, test_case in enumerate(pending_cases):
        rows.append({'date': today + timedelta(days=index), 'time': None, 'type': TestExecution.ExecutionType.NORMAL.label,
                     'project': test_case.test_plan.project, 'test_case': test_case, 'defect': None,
                     'reason': 'Caso pendiente de ejecucion', 'result': '', 'result_label': 'Pendiente',
                     'is_recorded': False, 'execution': None})
    confirmation_defects = Defect.objects.select_related('project', 'test_case').filter(
        project__in=projects, status__in=CONFIRMATION_CANDIDATE_STATUSES
    ).order_by('-updated_at')[:20]
    for index, defect in enumerate(confirmation_defects):
        rows.append({'date': today + timedelta(days=index + 1), 'time': None,
                     'type': TestExecution.ExecutionType.CONFIRMATION.label, 'project': defect.project,
                     'test_case': defect.test_case, 'defect': defect,
                     'reason': 'Confirmar corrección del defecto', 'result': '', 'result_label': 'Planificada',
                     'is_recorded': False, 'execution': None})
    regression_cases = TestCase.objects.select_related('test_plan', 'test_plan__project', 'requirement').filter(
        test_plan__project__in=projects, status=TestCase.Status.PASSED
    ...
    for index, test_case in enumerate(regression_cases):
        rows.append({'date': today + timedelta(days=index + 2), 'time': None,
                     'type': TestExecution.ExecutionType.REGRESSION.label, 'project': test_case.test_plan.project,
                     'test_case': test_case, 'defect': None,
                     'reason': 'Verificar que la corrección no afectó funcionalidad existente', 'result': '',
                     'result_label': 'Planificada', 'is_recorded': False, 'execution': None})
    return sorted(rows, key=lambda row: (row['date'], 0 if row['is_recorded'] else 1, row['project'].name, row['type']), reverse=True)
...
def execution_workspace_view(request):
    ...
    if is_teacher_user:
        ...
        selected_case = None
        if case_id:
            selected_case = TestCase.objects.select_related('requirement', 'test_plan', 'test_plan__project', 'created_by').filter(
                pk=case_id, test_plan__project__in=visible_projects_for(request.user, request=request)
            ).first()
    else:
        test_cases = TestCase.objects.select_related('requirement', 'test_plan', 'test_plan__project', 'created_by').order_by('test_plan__project__name', 'code')
        ...
        if case_id:
            ...
            selected_case = test_cases.filter(status=TestCase.Status.PENDING).first() or test_cases.first()
    step_results = []
    step_errors = []
    ...
    if request.method == 'POST' and is_teacher_user:
        execution = get_object_or_404(TestExecution, pk=request.POST.get('execution_id'),
        ...
        if review_form.is_valid():
            ...
            log_action(request.user, 'REVIEW', 'TestExecution', reviewed_execution.pk,
                       {'project_id': reviewed_execution.test_case.test_plan.project_id,
                        'test_case_id': reviewed_execution.test_case_id, 'review_status': reviewed_execution.review_status})
    ...
    if request.method == 'POST' and selected_case and form.is_valid():
        ...
        for step in step_results:
            TestStepExecution.objects.create(test_execution=execution, step_number=step['number'], action=step['action'],
                                             expected_result=step['expected_result'], obtained_result=step['actual_result'],
                                             status=step['status'], comment=step['comment'])
        log_action(request.user, 'CREATE', 'TestExecution', execution.pk,
                   {'project_id': selected_case.test_plan.project_id, 'test_case_id': selected_case.pk,
                    'result': execution.result, 'execution_type': execution.execution_type,
                    'related_defect_id': execution.related_defect_id, 'environment': execution.environment,
                    'started_at': execution.started_at.isoformat() if execution.started_at else None,
                    'finished_at': execution.finished_at.isoformat() if execution.finished_at else None})
        sync_defect_from_confirmation(execution)
        if execution.result == TestExecution.Result.FAILED and execution.execution_type != TestExecution.ExecutionType.CONFIRMATION:
        ...
        elif execution.result == TestExecution.Result.FAILED and execution.execution_type == TestExecution.ExecutionType.CONFIRMATION:
        ...
        return redirect(f'{request.path}?case={selected_case.id}')
    executions = selected_case.executions.select_related('executed_by').prefetch_related('automated_results__validation_rule', 'step_executions') if selected_case else TestCase.objects.none()
    manual_history = [{'execution': execution, 'evidence_is_image': is_image_evidence(execution.evidence)} for execution in executions if execution.execution_mode == TestExecution.ExecutionMode.MANUAL]
    automated_history = [{'execution': execution, 'evidence_is_image': is_image_evidence(execution.evidence)} for execution in executions if execution.execution_mode == TestExecution.ExecutionMode.AUTOMATED]
    ...
    manual_count = len(manual_history); automated_count = len(automated_history); execution_total = manual_count + automated_count
    passed_count = sum(1 for item in [*manual_history, *automated_history] if item['execution'].result == 'PASSED')
    failed_count = sum(1 for item in [*manual_history, *automated_history] if item['execution'].result == 'FAILED')
    ...
    automated_rules = selected_case.automated_rules.filter(is_active=True) if selected_case else AutomatedValidationRule.objects.none()
    ...
    return render(request, 'executions/index.html', {'form': form, 'selected_case': selected_case,
        ...
        'test_cases': test_cases if not is_teacher_user else TestCase.objects.none(), 'last_execution': last_execution,
        'review_form': review_form, 'last_execution_evidence_is_image': is_image_evidence(last_execution.evidence) if last_execution else False,
        'execution_history': [*manual_history, *automated_history], 'manual_history': manual_history, 'automated_history': automated_history,
        'manual_count': manual_count, 'automated_count': automated_count, 'execution_total': execution_total,
...
def execution_history_view(request, case_id):
    ...
    test_cases = test_cases.filter(test_plan__project__in=visible_projects_for(request.user, request=request))
    test_case = get_object_or_404(TestCase.objects.select_related('requirement', 'test_plan', 'test_plan__project', 'created_by'), pk=case_id,
                                  test_plan__project__in=visible_projects_for(request.user, request=request))
    executions = test_case.executions.select_related('executed_by').prefetch_related('automated_results__validation_rule', 'step_executions')
    manual_history = [{'execution': execution, 'evidence_is_image': is_image_evidence(execution.evidence)} for execution in executions if execution.execution_mode == TestExecution.ExecutionMode.MANUAL]
    automated_history = [{'execution': execution, 'evidence_is_image': is_image_evidence(execution.evidence)} for execution in executions if execution.execution_mode == TestExecution.ExecutionMode.AUTOMATED]
    total = len(manual_history) + len(automated_history)
    passed = sum(1 for item in [*manual_history, *automated_history] if item['execution'].result == TestExecution.Result.PASSED)
    failed = sum(1 for item in [*manual_history, *automated_history] if item['execution'].result == TestExecution.Result.FAILED)
    success_percent = round((passed / total) * 100) if total else 0
    return render(request, 'executions/history.html', {'case': test_case, 'test_cases': test_cases,
        'manual_history': manual_history, 'automated_history': automated_history, 'manual_count': len(manual_history),
        'automated_count': len(automated_history), 'execution_total': total, 'passed_count': passed,
        'failed_count': failed, 'success_percent': success_percent, 'can_manage': can_manage_artifacts(request.user)})


@login_required
def execution_detail_view(request, pk):
    execution = get_object_or_404(TestExecution.objects.select_related('test_case', 'test_case__test_plan', 'test_case__test_plan__project', 'executed_by', 'reviewed_by', 'related_defect').prefetch_related('step_executions', 'automated_results__validation_rule', 'defects'),
                                 pk=pk, test_case__test_plan__project__in=visible_projects_for(request.user, request=request))
    step_executions = list(execution.step_executions.all())
    step_rows = [{'step': step, 'review_form': StepReviewForm(instance=step)} for step in step_executions]
    return render(request, 'executions/detail.html', {'execution': execution, 'step_executions': step_executions, 'step_rows': step_rows,
        'automated_results': execution.automated_results.all(), 'defects': execution.defects.all(),
        'can_upload_step_evidence': (not is_teacher(request.user) and execution.review_status == TestExecution.ReviewStatus.PENDING and (request.user.is_superuser or execution.executed_by_id == request.user.id)),
        'can_manage': can_manage_artifacts(request.user), 'is_teacher': is_teacher(request.user),
...
def execution_review_detail_view(request, pk):
    execution = get_object_or_404(TestExecution.objects.select_related('test_case', 'test_case__test_plan__project'), pk=pk,
                                  test_case__test_plan__project__in=visible_projects_for(request.user, request=request))
