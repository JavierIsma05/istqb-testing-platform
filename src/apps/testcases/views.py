...
def testcase_list_view(request):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        test_case.code = next_code(TestCase.objects.filter(test_plan=test_case.test_plan), 'TC')
    ...
    test_cases = TestCase.objects.select_related('test_plan', 'test_plan__project', 'requirement', 'created_by').prefetch_related('covered_risks').annotate(
    ...
    if query:
        test_cases = test_cases.filter(Q(code__icontains=query) | Q(title__icontains=query) | Q(description__icontains=query) | Q(expected_result__icontains=query))
    if active_project:
...
def testcase_detail_view(request, pk):
    test_case = get_object_or_404(
        TestCase.objects.select_related('test_plan', 'test_plan__project', 'requirement', 'created_by').prefetch_related('defects', 'executions__defects', 'traceability_links', 'covered_risks'),
        pk=pk,
...
def testcase_update_view(request, pk):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        log_action(request.user, 'UPDATE', 'TestCase', test_case.pk, {'project_id': test_case.test_plan.project_id, 'test_plan_id': test_case.test_plan_id, 'requirement_id': test_case.requirement_id, 'code': test_case.code, 'title': test_case.title})
...
def testcase_delete_view(request, pk):
    ...
    if request.method == 'POST':
        has_history = test_case.versions.exists()
        has_executions = test_case.executions.exists()
        has_defects = test_case.defects.exists()
        has_traceability = test_case.traceability_links.exists()
        if has_history or has_executions or has_defects or has_traceability:
            messages.error(request, f'No se puede eliminar {test_case.code} porque forma parte del historial, las ejecuciones, los defectos o la trazabilidad. Conserva el caso y actualízalo en lugar de borrarlo.')
            return redirect('testcases:detail', pk=test_case.pk)
        log_action(request.user, 'DELETE', 'TestCase', test_case.pk, {'project_id': test_case.test_plan.project_id, 'test_plan_id': test_case.test_plan_id, 'requirement_id': test_case.requirement_id, 'code': test_case.code, 'title': test_case.title})
        test_case.delete()
