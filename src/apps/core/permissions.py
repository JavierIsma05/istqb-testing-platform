...
def can_manage_project(user, project):
    return is_admin(user) or (is_student(user) and project.created_by_id == user.id)
...
def get_active_project_for_request(request, user=None):
    ...
    project_id = request.GET.get('project', '').strip() or request.POST.get('project', '').strip() or request.session.get('active_project_id')
    if project_id:
        try:
...
def visible_projects_for(user, request=None):
    ...
    if request is not None:
        # Una URL con un caso explícito ya identifica el contexto de proyecto.
        # No debe quedar oculta por un active_project_id antiguo de la sesión.
...
def redirect_if_teacher_readonly(request, route_name, module_name):
    if not is_teacher(request.user):
        return None
    messages.warning(request, f'Los docentes solo pueden revisar {module_name} en modo lectura.')
